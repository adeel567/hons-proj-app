 ...
 * @param {*} props navigation and item to be rendered
 * @returns
 */
  ...
  return (
    <View style={{ marginHorizontal: 25, marginVertical: 10 }}>
      ...
      >
        <Card.Title
          title={item.name}
          subtitle={"Sold by " + item.restaurant_name}
        />
        <Card.Content style={{ alignItems: "flex-end" }}>
          <Paragraph>£{Number(item.pence / 100).toFixed(2)}</Paragraph>
        </Card.Content>
        <Card.Actions style={{ marginTop: -39 }}>
          <AddToCartButton navigation={navigation} id={item.id}>
            Add another
          </AddToCartButton>
          <RemoveFromCartButton navigation={navigation} id={item.id}>
            Remove
