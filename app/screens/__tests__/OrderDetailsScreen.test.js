...
it("All elements exist", async() => {
    renderScreen() 
    await waitFor(() => { //basic check for all components to be rendered.
...
it("On api failure, a safe error of just loading with no elements should occur", async () => {
    ...
    axiosInstance.get.mockRejectedValue({response:{ status:404, data: {"res": "error"} }});
    ...
    await waitFor(() => {
        ...
        expect(Alert.alert).toHaveBeenCalledWith("An error occurred :(", expect.anything(), expect.anything())
