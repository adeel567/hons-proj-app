...
const renderScreen = () => {
  return render(<VendorAboutScreen navigation={navigation} route={route} />);
};
...
it("Test if marker is correct", async () => {
  renderScreen();
  await waitFor(() => {
    expect(screen.getByTestId("marker")).toHaveProp("coordinate", {
      latitude: vendor.latitude,
      longitude: vendor.longitude,
    });
    expect(screen.getByTestId("marker")).toHaveProp("title", vendor.name);
  });
});
