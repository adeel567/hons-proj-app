...
it("Clicking on about should take you to about screen for that vendor", async () => {
    ...
    await waitFor(() => {
        ...
        expect(navigation.navigate).toHaveBeenCalledWith("About", expect.anything())
...
it("Searching should work and render only valid results", async () => {
    renderScreen()
    await waitFor(() => {
        fireEvent.changeText(screen.getAllByPlaceholderText("Search for menu items.")[0],"Chargrill chicken oval bite") 
...
it("Search on non-existing should return empty message", async () => {
    renderScreen()
    await waitFor(() => {
        fireEvent.changeText(screen.getAllByPlaceholderText("Search for menu items.")[0],"garbagestring") 
        expect(screen.queryByText("Chicken club baguette")).toBeNull()
...
it("Reset search should restore missing items", async () => {
    ...
    await waitFor(() => {
        ...
        expect(screen.queryByText("Chargrill chicken oval bite")).toBeNull() 
        screen.getByText("No items found!")
        fireEvent.changeText(screen.getAllByPlaceholderText("Search for menu items.")[0],"") 
...
it("Pressing sort should do sort (name desc)", async () => {
    ...
    await waitFor(() => {
        ...
        expect(screen.getAllByTestId("itemTitle")[0]).toHaveTextContent("Chicken club baguette");
    })
})
...
it("Pressing sort should do sort (name asc)", async () => {
    ...
    await waitFor(() => {
        ...
        expect(screen.getAllByTestId("itemTitle")[0]).toHaveTextContent("Chargrill chicken oval bite");
    })
})
