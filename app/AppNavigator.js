...
export const AppStack = () => {
  const { cartBadge, setTriggerOrderRefresh, triggerOrderRefresh } =
  ...
  async function registerForPush() {
    ...
    if (Device.isDevice) {
      ...
      if (typeof token !== "undefined") {
        axiosInstance.post("/profile/push-token", params).catch((error) => {
...
const CartStack = () => {
  return (
    <Stack.Navigator>
      ...
      />
      <Stack.Screen
        name="Choose Delivery Location"
        component={ChooseDeliveryLocation}
      />
    </Stack.Navigator>
  );
};
